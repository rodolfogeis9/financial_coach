...
export const VariableExpensesSection = () => {
  ...
  const totalVariables =
    variableBlocks.reduce((sum, block) => sum + blockSubtotal(block.items), 0) +
    otrosGastos.reduce((sum, gasto) => sum + gasto.monto_mensual, 0);
  ...
  return (
    <div className="space-y-6">
      ...
      <Card title="Otros gastos variables" actions={<span className="text-sm font-semibold text-slate-500">Subtotal: ${otrosGastos.reduce((sum, gasto) => sum + gasto.monto_mensual, 0).toLocaleString('es-CL')}</span>}>
      ...
      <Card title="Resumen">
        <p className="text-sm text-slate-500">Total gastos variables</p>
        <p className="text-3xl font-bold text-slate-900">${totalVariables.toLocaleString('es-CL')}</p>
      </Card>
    </div>
  );
};
